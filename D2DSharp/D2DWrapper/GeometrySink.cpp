...
{
	GetNative<ID2D1GeometrySink>()->AddArc((D2D1_ARC_SEGMENT*)&arc);
}

void GeometrySink::AddBezier(BezierSegment bezier)
{
	GetNative<ID2D1GeometrySink>()->AddBezier((D2D1_BEZIER_SEGMENT*) &bezier);
}

void GeometrySink::AddLine(PointF point)
{
	GetNative<ID2D1GeometrySink>()->AddLine(*(D2D1_POINT_2F*) &point);
}

void GeometrySink::AddQuadraticBezier(QuadraticBezierSegment bezier)
{
	GetNative<ID2D1GeometrySink>()->AddQuadraticBezier((D2D1_QUADRATIC_BEZIER_SEGMENT *) &bezier);
}
...
void GeometrySink::AddQuadraticBeziers(array<QuadraticBezierSegment>^ beziers)
{
	...
	GetNative<ID2D1GeometrySink>()->AddQuadraticBeziers((D2D1_QUADRATIC_BEZIER_SEGMENT *) p, beziers->Length);
}
